...
class FastRobotController(Node):
    """Extended controller for the robot's movement with more decision points and areas."""
    
    def __init__(self):
        super().__init__('fast_robot_controller')
        
        # Parameters
        self.declare_parameter('linear_speed', 1.0)      # Default much faster than before
        self.declare_parameter('angular_speed', 1.2)     # Default much faster than before
        self.declare_parameter('obstacle_threshold', 0.5)
        self.declare_parameter('goal_tolerance', 0.3)     # Increased tolerance
        self.declare_parameter('decision_point_x', 10.0)  # Updated from world file
        self.declare_parameter('decision_point_y', 0.0)
        self.declare_parameter('area1_x', 25.0)          # Much further away
        self.declare_parameter('area1_y', 30.0)
        self.declare_parameter('area2_x', 25.0)          # Much further away
        self.declare_parameter('area2_y', -30.0)
        ...
        self.area3_position = [40.0, -15.0]  # Hospital area (far to the right)
        self.area4_position = [16.0, 35.0]   # School area (up north)
        
        # Secondary decision points - positioned along the paths to Area 1 and Area 2
        self.secondary_decision_point_area1 = [16.0, 15.0]  # Secondary decision on way to Area 1
        self.secondary_decision_point_area2 = [16.0, -15.0]  # Secondary decision on way to Area 2
        ...
        self.control_timer = self.create_timer(0.05, self.control_loop)  # Faster control loop (20Hz)
        
        self.get_logger().info('Fast Robot Controller initialized')
        self.publish_state()
    ...
    def control_loop(self):
        ...
        # Log current state and position (less frequently to reduce console spam)
        if int(time.time() * 2) % 2 == 0:  # Log roughly every 0.5 seconds
        ...
        if self.current_state in [RobotState.WAITING_FOR_DECISION, RobotState.AT_SECONDARY_DECISION]:
            ...
            if self.at_goal():
                ...
                if abs(angle_diff) < 0.3:  # About 17 degrees
                    ...
                    twist.angular.z = angle_diff * 0.7  # Increased correction factor
                else:
            ...
            if self.current_operation_start_time is not None:
                ...
                rescue_time = 3.0  # Faster default rescue time
                
                # Different rescue times based on the area (shorter than before)
                if self.ethical_decision == "area1" and self.secondary_choice != "area3" and self.secondary_choice != "area4":
                    rescue_time = 6.0  # Was 12.0
                elif self.ethical_decision == "area2" and self.secondary_choice != "area3" and self.secondary_choice != "area4":
                    rescue_time = 3.0  # Was 5.0
                elif self.secondary_choice == "area3":
                    rescue_time = 4.0  # Was 8.0
                elif self.secondary_choice == "area4":
                    rescue_time = 5.0  # Was 10.0
...
def main(args=None):
    ...
    robot_controller = FastRobotController()
